...
public class GanttV3Phase3T9Tests : IAsyncLifetime
{
    ...
    [Fact]
    public void InfiniteScroll_Uncontrolled_Default_Matches_GanttCharts_Own_True_Default()
    {
    ...
    // ── GanttChart/GanttTimeline: the extension pipeline itself ─────────────────

    // Every GanttChart instance performs its OWN scroll-to-today/center attempt on
    ...
    private async Task<IRenderedComponent<L.GanttChart>> RenderChart(bool? infiniteScroll = null)
    {
        var cut = _ctx.Render<L.GanttChart>(p =>
        {
    ...
    public async Task Scrolling_Near_The_Leading_Edge_Extends_VisibleRange_Backward_By_Exactly_One_Page_And_Recenters()
    {
        ...
        // GanttChart.VisibleCenterDate's own formula — ResolveCurrentCenterDateAsync's
    ...
    // button (GanttChart's own GoToTodayAsync, exactly what a live NavTests spec
    ...
    // GanttChart.HandleRangeExtensionRequestAsync's OWN independent `!InfiniteScroll`
    ...
    // GanttChart's later one, can produce.
    [Fact]
    ...
    // put — GanttChart's OWN independent re-check (decision 3's second layer,
    ...
    // is specifically what stops GanttChart from EVER attempting its own
    // live-center capture in the first place (see TryRequestRangeExtensionAsync's
    // own remarks — "avoids even asking GanttChart") — a signal ONLY the cheap
    // gate, not GanttChart's later re-check, can produce, since by the time
    // GanttChart's re-check runs the capture has ALREADY happened.
    [Fact]
    public async Task A_Report_While_A_Drag_Is_Active_Never_Extends()
    {
        ...
        Assert.Equal(oldStart, timeline.Instance.RangeStart);
        // The cheap gate short-circuits BEFORE ever reaching GanttChart's own
        ...
        // The precise signal that isolates THIS layer from GanttChart's own
    ...
    // GanttChart's actual commit. GanttChart's OWN re-check (HandleRangeExtensionRequestAsync's
    ...
    public async Task A_Drag_That_Starts_During_The_Live_Center_Capture_Aborts_The_Pending_Extension()
    {
        ...
        // passes (false), GanttChart.HandleRangeExtensionRequestAsync starts its
        ...
        // Resume the capture — GanttChart's own re-check (immediately after) now
        // observes the drag and abandons instead of committing.
        await cut.InvokeAsync(() => gate.SetResult(0));

        Assert.Equal(oldStart, timeline.Instance.RangeStart);
        Assert.Equal(2, _interop.GanttV3HasActiveDragCallCount); // GanttTimeline's initial check + GanttChart's own re-check
    }
    ...
    // in GanttChart's own async pipeline (not merely GanttTimeline's own cheap
    ...
    public async Task A_Second_Overlapping_Report_Is_Dropped_While_The_First_Extension_Is_Still_In_Flight()
    {
        ...
        // First report: suspends inside GanttChart's OWN pipeline (past
