...
public class GanttV3ProgressClickCanDropTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task GanttChart_Progress_Commit_Fires_OnProgressChange_Not_OnDateChange()
    {
    ...
    [Fact]
    public async Task GanttChart_Drag_Commit_Still_Fires_OnDateChange_Not_OnProgressChange()
    {
    ...
    [Fact]
    public async Task GanttChart_OnTaskClick_Passthrough_Fires_With_The_Task()
    {
        L.GanttTask? clicked = null;
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.Tasks, new List<L.GanttTask> { Task1() })
    ...
    [Fact]
    public void GanttChart_CanDrop_Passthrough_Reaches_The_Nested_Timeline()
    {
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.Tasks, new List<L.GanttTask> { Task1() })
            .Add(c => c.CanDrop, (L.GanttTask _, GanttScheduleDropContext _) => false));
