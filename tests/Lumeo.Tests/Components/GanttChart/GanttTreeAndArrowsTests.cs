...
/// and the additions to <see cref="L.GanttTimeline"/>/<see cref="L.GanttChart"/> that
...
public class GanttTreeAndArrowsTests : IAsyncLifetime
{
    ...
    // ── GanttChart: ShowTreePane wiring ──────────────────────────────────────────

    [Fact]
    public void GanttChart_Does_Not_Render_The_Tree_Pane_By_Default_For_A_Flat_Task_List()
    {
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, new[]
        {
            new L.GanttTask("t1", "Design", D(2026, 1, 1), D(2026, 1, 5)),
        }));
    ...
    [Fact]
    public void GanttChart_Renders_The_Tree_Pane_By_Default_When_A_Task_Has_A_ParentId()
    {
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, new[]
        {
            new L.GanttTask("parent", "Parent", D(2026, 1, 1), D(2026, 1, 5)),
    ...
    [Fact]
    public void GanttChart_ShowTreePane_False_Overrides_The_Hierarchy_Default()
    {
    ...
    [Fact]
    public void GanttChart_ShowTreePane_True_Overrides_The_Flat_Default()
    {
        var cut = _ctx.Render<L.GanttChart>(p => p
