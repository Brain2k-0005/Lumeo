...
public class GanttV3CodexRound9Tests : IAsyncLifetime
{
    ...
    private static string PeriodLabel(IRenderedComponent<L.GanttChart> cut) =>
    ...
    // ── GanttChart.ComputeInitialRange: shared normalize+filter step (P2 #2/#3) ─

    [Fact]
    public void GanttChart_VisibleRange_Is_Identical_For_A_Timed_Task_And_Its_Midnight_Twin()
    {
        ...
        var timedCut = timedCtx.Render<L.GanttChart>(p => p
        ...
        var midnightCut = midnightCtx.Render<L.GanttChart>(p => p
    ...
    [Fact]
    public void GanttChart_VisibleRange_Excludes_A_Filtered_Invalid_Task_From_Its_Min_Max()
    {
        ...
        var soleCut = soleCtx.Render<L.GanttChart>(p => p
        ...
        var withInvalidCut = withInvalidCtx.Render<L.GanttChart>(p => p
    ...
    // ── GanttChart: recenter on tasks (not Today) when Today falls outside the
    //    async-populated range (P2 #5) ───────────────────────────────────────

    [Fact]
    public void GanttChart_Centers_On_The_Tasks_Not_Today_When_Async_Populated_Tasks_Are_Far_From_Today()
    {
    ...
    [Fact]
    public void GanttChart_Still_Centers_On_Today_When_Async_Populated_Tasks_Do_Contain_Today()
    {
    ...
    // ── GanttChart: direction changes via the global IThemeService (P2 #7) ─────

    [Fact]
    public async Task GanttChart_Requests_A_Recenter_When_The_Global_ThemeService_Direction_Changes_Without_A_DirectionProvider()
    {
        ...
        // the flip went unnoticed. GanttChart now subscribes directly.
    ...
    [Fact]
    public async Task GanttChart_Unsubscribes_From_The_ThemeService_On_Dispose()
    {
