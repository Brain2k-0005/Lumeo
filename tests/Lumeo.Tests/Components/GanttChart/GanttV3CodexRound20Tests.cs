...
///
///  A (P1, GanttChart.razor line ~526) — "Recheck supersession after awaiting the
...
///
///  B (P1, GanttChart.razor line ~501) — "Discard parameters superseded by a
...
public class GanttV3CodexRound20Tests : IAsyncLifetime
{
    ...
    public async Task GoToTodayAsync_Rechecks_Supersession_After_Awaiting_A_Controlled_Mode_Callback()
    {
        ...
        var handleViewModeChanged = typeof(L.GanttChart).GetMethod("HandleViewModeChangedAsync", BindingFlags.NonPublic | BindingFlags.Instance)!;
