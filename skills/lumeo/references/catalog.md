...
- **Mail** (`/blocks/mail`) — shadcn's Mail example, ported one to one: three resizable panes with an account switcher and a collapsible folder nav, an inbox list with All/Unread tabs and search, and a reading pane with archive, snooze, reply and forward actions and a reply composer.
- **Music** (`/blocks/music`) — shadcn's Music example, ported one to one: a five-menu Menubar, a library sidebar with a scrolling playlist list, Music and Podcasts tabs, album rows with a ContextMenu, and an Add Podcast dialog.
