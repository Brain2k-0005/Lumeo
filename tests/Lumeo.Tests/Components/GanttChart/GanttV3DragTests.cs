...
/// GanttTaskUpdate -> GanttChart state-merge pipeline. gantt-v3.js's pointer/ghost
...
public class GanttV3DragTests : IAsyncLifetime
{
    ...
    // ── GanttChart end-to-end (bubbled through the real, nested GanttTimeline) ───

    [Fact]
    public async Task GanttChart_Drag_Commit_Merges_Task_And_Fires_TasksChanged_OnDateChange_OnTaskUpdate()
    {
    ...
    // ── Phase 4 — the OnTaskUpdate commit gate (reject/accept/adjust) ────────

    [Fact]
    public async Task GanttChart_OnTaskUpdate_Reject_Commits_Nothing()
    {
        // Predicted (against the broken pre-Phase-4 state, where OnTaskUpdate
        // was a post-commit EventCallback<GanttTaskUpdate> notification):
        // returning `false` from a Func<GanttTaskUpdate, GanttUpdateResult>
        // wasn't even expressible then — the drag committed UNCONDITIONALLY,
        // so GanttState.Tasks[0].Start would read 2026-01-05 (the dragged
        // date) and TasksChanged/OnDateChange would both fire. Against the
        // real Phase-4 gate, none of that happens: Start stays 2026-01-02
        // (Task1's original), and every downstream callback is silent.
        ...
        var cut = _ctx.Render<L.GanttChart>(p => p
            ...
            .Add(c => c.OnTaskUpdate, (GanttTaskUpdate _) => false));
        ...
        var committed = Assert.Single(State(cut).Tasks);
        Assert.Equal(D(2026, 1, 2), committed.Start); // unchanged — Task1's original Start
        Assert.Equal(D(2026, 1, 6), committed.End);   // unchanged — Task1's original End

        // The rendered bar was never touched either.
        Assert.Equal("2026-01-02", cut.Find("[data-task-id='t1']").GetAttribute("data-task-start"));
    }

    [Fact]
    public async Task GanttChart_OnTaskUpdate_AcceptWith_Commits_The_Adjusted_Dates_Not_The_Raw_Proposal()
    {
        // Predicted (against a naive "adjustment is ignored, always commits
        // the raw proposal" bug — the failure mode a partial gate
        // implementation would produce): the committed/rendered Start would
        // read the RAW proposed "2026-01-05", not the snapped "2026-01-06"
        // this gate asks for. Asserting the SNAPPED value fails loudly
        // against that broken behaviour instead of silently passing either
        // way.
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.Tasks, new List<L.GanttTask> { Task1 })
            .Add(c => c.OnTaskUpdate, (GanttTaskUpdate u) => GanttUpdateResult.AcceptWith(
                new GanttUpdateAdjustment(Start: D(2026, 1, 6), End: D(2026, 1, 10)))));

        var timeline = cut.FindComponent<L.GanttTimeline>();
        await cut.InvokeAsync(() => timeline.Instance.CommitDrag("t1", "move", "2026-01-05", "2026-01-09"));

        var committed = Assert.Single(State(cut).Tasks);
        Assert.Equal(D(2026, 1, 6), committed.Start);  // the ADJUSTMENT's Start, not the proposed 2026-01-05
        Assert.Equal(D(2026, 1, 10), committed.End);   // the ADJUSTMENT's End, not the proposed 2026-01-09

        Assert.Equal("2026-01-06", cut.Find("[data-task-id='t1']").GetAttribute("data-task-start"));
        Assert.Equal("2026-01-10", cut.Find("[data-task-id='t1']").GetAttribute("data-task-end"));
    }

    [Fact]
    public async Task GanttChart_OnTaskUpdate_AcceptWith_Progress_Adjustment_Commits_The_Adjusted_Percentage()
    {
        // Same "adjustment must actually apply" contract as the Start/End
        // test above, exercised for Progress — the field ReUI's own
        // {start?, end?, allDay?} shape has no analog for (Gantt has no
        // AllDay concept; Progress is the field that fills its place here,
        // see GanttUpdateAdjustment's own remarks).
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.Tasks, new List<L.GanttTask> { Task1 with { Progress = 10 } })
            .Add(c => c.OnTaskUpdate, (GanttTaskUpdate u) => GanttUpdateResult.AcceptWith(
                new GanttUpdateAdjustment(Progress: 50))));

        var timeline = cut.FindComponent<L.GanttTimeline>();
        await cut.InvokeAsync(() => timeline.Instance.CommitProgress("t1", 77));

        var committed = Assert.Single(State(cut).Tasks);
        Assert.Equal(50, committed.Progress); // the ADJUSTMENT's Progress, not the raw 77 the drag proposed
    }

    [Fact]
    public async Task GanttChart_Uncontrolled_Drag_Commit_Survives_An_Unrelated_Rerender_With_Stale_Tasks_Parameter()
    {
        // Regression guard for the discriminator this task adds to
        // GanttChart.OnParametersSet (mirrors v2 Gantt.razor's
        ...
        // every OnParametersSet, so any later re-render that handed GanttChart the
        ...
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, initialTasks));
    ...
    [Fact]
    public async Task GanttChart_Controlled_Drag_Commit_Rolls_Back_On_A_Genuine_Parent_Veto()
    {
    ...
    // ── Codex PR-383 findings (GanttChart.razor, blocked until PR #382 merged) ───

    [Fact]
    public async Task GanttChart_Drag_Commit_Expands_VisibleRange_To_Cover_A_Task_Moved_Outside_It()
    {
        ...
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, new List<L.GanttTask> { Task1 }));
    ...
    [Fact]
    public async Task GanttChart_Drag_Commit_Fires_Edit_Callbacks_Before_TasksChanged()
    {
        // [P2] "Notify edit callbacks before TasksChanged" — v2 parity requires
        // the gesture-specific edit callbacks (OnDateChange/OnProgressChange/
        // OnTaskCreate) to fire BEFORE TasksChanged, not after: a controlled
        // parent's TasksChanged handler can synchronously normalize/veto and
        // rerender, so anything that runs afterward only ever observes
        // whatever the chart already adopted or rolled back to.
        //
        // Phase 4 — OnTaskUpdate is now the PRE-COMMIT GATE (see
        // GanttUpdateResult's own remarks), so it necessarily runs FIRST,
        // before anything commits at all — not "alongside" OnDateChange
        // anymore, ahead of it. Order is gate -> commit -> per-source
        ...
        var cut = _ctx.Render<L.GanttChart>(p => p
            ...
            .Add(c => c.OnTaskUpdate, (GanttTaskUpdate _) => { order.Add("OnTaskUpdate"); return true; }));

        var timeline = cut.FindComponent<L.GanttTimeline>();
        await cut.InvokeAsync(() => timeline.Instance.CommitDrag("t1", "move", "2026-01-05", "2026-01-09"));

        Assert.Equal(new[] { "OnTaskUpdate", "OnDateChange", "TasksChanged" }, order);
    }

    [Fact]
    public void GanttChart_Readonly_Suppresses_Drag_Registration_On_The_Nested_Timeline()
    {
        _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.Tasks, new List<L.GanttTask> { Task1 })
            .Add(c => c.Readonly, true));
    ...
    [Fact]
    public void GanttChart_Not_Readonly_Registers_Drag_Interop_On_The_Nested_Timeline()
    {
        _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, new List<L.GanttTask> { Task1 }));
    ...
    [Fact]
    public async Task GanttChart_Concurrent_Drag_Commits_On_Different_Bars_Both_Survive_A_Range_Expansion_Capture()
    {
        ...
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, new List<L.GanttTask> { taskA, taskB }));
        var timeline = cut.FindComponent<L.GanttTimeline>();
    ...
    [Fact]
    public async Task GanttChart_Bar_Click_Stays_Native_When_Drag_Registration_Throws()
    {
