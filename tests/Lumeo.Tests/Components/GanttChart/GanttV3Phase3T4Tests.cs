...
/// This file proves a REAL <see cref="L.GanttChart"/> fulfills the
...
/// <see cref="L.GanttChart"/> instances sharing ONE <see cref="GanttState"/> stay in
...
public class GanttV3Phase3T4Tests : IAsyncLifetime
{
    ...
    {
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, Fixture()));
    ...
    [Fact]
    public async Task ScrollToDateAsync_On_A_Hoisted_GanttChart_Issues_A_Real_GanttV3ScrollToXAsync_Call()
    {
        ...
        Assert.True(_interop.GanttV3ScrollToXCallCount > callsBefore,
            "expected ScrollToDateAsync to issue a NEW GanttV3ScrollToXAsync call through the attached GanttChart's existing scroll interop");
    }

    [Fact]
    public async Task ScrollToTaskAsync_On_A_Hoisted_GanttChart_Scrolls_Toward_The_Tasks_Own_Midpoint()
    {
        var state = new GanttState();
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.State, state)
    ...
    [Fact]
    public async Task SetViewMode_On_An_Uncontrolled_Hoisted_GanttChart_Updates_The_Rendered_Mode()
    {
        var state = new GanttState();
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.State, state)
        ...
        // dispatch is InvokeAsync'd onto GanttChart's own sync context — awaiting
    ...
    [Fact]
    public async Task SetViewMode_On_A_Controlled_Hoisted_GanttChart_Echoes_Through_ViewModeChanged_Exactly_Like_A_Toolbar_Pick()
    {
    ...
    [Fact]
    public async Task After_The_Attached_GanttChart_Is_Disposed_The_States_Imperative_Api_Is_A_Silent_No_Op()
    {
        var state = new GanttState();
        var cut = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.State, state)
        ...
        cut.Instance.Dispose(); // GanttChart's own IDisposable.Dispose() -> state.Detach(this)
    ...
    [Fact]
    public void A_Nav_Click_In_One_GanttChart_Instance_Updates_A_Sibling_Sharing_The_Same_State()
    {
        var state = new GanttState();
        var cutA = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.State, state)
            .Add(c => c.Tasks, Fixture()));
        var cutB = _ctx.Render<L.GanttChart>(p => p
            .Add(c => c.State, state)
            .Add(c => c.Tasks, Fixture()));
    ...
    private static string ReadPeriodLabel(IRenderedComponent<L.GanttChart> cut)
    {
