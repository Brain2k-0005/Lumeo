...
public class GanttV3Phase3T8Tests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task ShowOffscreenIndicators_Uncontrolled_Default_Matches_GanttCharts_Own_True_Default()
    {
        var cut = _ctx.Render<L.GanttSettingsMenu>();
        cut.Find("button[aria-label='Settings']").Click();
    ...
    public async Task FirstDayOfWeek_Controlled_At_A_NonNull_Value_Vetoes_A_Toggle_Attempt()
    {
        ...
        // limitation GanttChart.TreePaneWidth's own identical idiom has), so a
    ...
    // ── GanttNav.TrailingContent / GanttChart.TrailingContent passthrough ──────
    ...
    [Fact]
    public void GanttChart_Forwards_TrailingContent_Through_To_GanttNav()
    {
    ...
    {
        _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, Fixture()));
