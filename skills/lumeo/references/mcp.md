...
The 16 full-page "blocks" (dashboard, authentication, calendar, chat, ecommerce, file-manager, filters, form-wizard, kanban, mail, music, notifications, settings, social-feed, tasks, analytics). `list` → `[{ title, key, route, description }]`. `get` → `{ title, route, description, examples: [{ title, code }] }`. Great starting skeletons for a real page.
