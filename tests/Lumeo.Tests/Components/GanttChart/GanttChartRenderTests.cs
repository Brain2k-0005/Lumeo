...
/// marker, row virtualization), and <see cref="L.GanttChart"/> (root
...
/// </summary>
public class GanttChartRenderTests : IAsyncLifetime
{
    private readonly BunitContext _ctx = new();

    public GanttChartRenderTests() => _ctx.AddLumeoServices();
    ...
    [Fact]
    public void GanttChart_Renders_Nav_And_Timeline_Without_Throwing_For_Default_And_Task_Backed_Fixtures()
    {
        var exception = Record.Exception(() => _ctx.Render<L.GanttChart>());
        ...
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, tasks));
        Assert.Contains("Design", cut.Markup);
        Assert.Contains("Kickoff", cut.Markup);
    }

    [Fact]
    public void GanttChart_Recomputes_VisibleRange_When_An_Entirely_New_Task_Set_Arrives()
    {
    ...
    [Fact]
    public void GanttChart_Class_And_AdditionalAttributes_Splat_Onto_The_Root()
    {
    ...
    [Fact]
    public void GanttChart_Renders_The_ZoomLevel_Toolbar_Buttons()
    {
        var cut = _ctx.Render<L.GanttChart>();
