...
public class GanttV3Phase3T7Tests : IAsyncLifetime
{
    ...
    private static string? BarColorStyle(IRenderedComponent<L.GanttChart> cut, string taskId)
    {
    ...
    [Fact]
    public async Task ColorByGroup_Survives_A_Real_Row_Reorder_Through_GanttChart()
    {
    ...
    // of "near" regardless of where GanttChart's own ComputeInitialRange happens
    ...
    private static (double X, double Width) BarGeometryOf(IRenderedComponent<L.GanttChart> cut, string taskId)
    {
    ...
    {
        var cut = _ctx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, OffscreenFixture()));
    ...
    // scroll host, which in hosted GanttChart (tree pane shown) also covers the
