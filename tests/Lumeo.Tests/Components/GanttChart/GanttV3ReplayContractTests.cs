...
///
///  X (P2, GanttChart.razor:499) — Tasks is an IEnumerable, so it may be a
...
///  Y (P1, GanttChart.razor:561) — a parameter pass that says nothing about the mode
...
///      navigation owes.
///  Z (P2, GanttChart.razor:1165) — ShiftAsync re-applies its page step on top of a
...
public class GanttV3ReplayContractTests : IAsyncLifetime
{
    ...
    private static Task InvokeAsync(IRenderedComponent<L.GanttChart> cut, string method) =>
    ...
    public async Task A_Non_Claiming_Parameter_Pass_That_Commits_Still_Lands_A_Pending_Toolbar_Mode(string change)
    {
        ...
        {
            [nameof(L.GanttChart.Tasks)] = change == "tasks"
                ? new List<L.GanttTask> { taskB }
                : new List<L.GanttTask> { taskA },
            [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Day, // unchanged — this pass claims nothing
            [nameof(L.GanttChart.ShowTreePane)] = change == "geometry",
        })));
    ...
    public async Task A_Parameter_Pass_With_Nothing_To_Do_Still_Leaves_A_Suspended_Mode_Reconcile_Alone()
    {
        ...
        await cut.InvokeAsync(() => cut.Instance.SetParametersAsync(ParameterView.FromDictionary(new Dictionary<string, object?>
        {
            ...
            [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Day,
            [nameof(L.GanttChart.ShowTreePane)] = false,
            [nameof(L.GanttChart.Class)] = "different",
        })));
