...
/// <summary>
/// Triage #145 (low, state-on-data-change) — the ripple pointerdown listener was
/// attached only inside <c>if (firstRender &amp;&amp; PressEffect == Ripple)</c>, with no
/// <c>OnParametersSet</c> reconciliation and no dispose. So a runtime
/// <see cref="Lumeo.Button.ButtonPressEffect"/> <c>None -&gt; Ripple</c> flip never attached the
/// listener, a <c>Ripple -&gt; None</c> flip left a leaked listener bound, and tearing the
/// Button down never detached it (subsumes the #22 missing-dispose leak).
///
/// bUnit can't observe the real DOM listener, so the testable seam is the .NET wiring:
/// the attach/detach must be reconciled against the live <c>PressEffect</c> on every render
/// and torn down on dispose. These tests re-render via <c>cut.Render(p =&gt; p.Add(...))</c>
/// and assert on the recorded RippleAttach/RippleDetach calls captured by
/// <see cref="TrackingInteropService"/>.
/// </summary>
public class ButtonPressEffectReconcileTests : IAsyncLifetime
{
    private readonly BunitContext _ctx = new();
    private readonly TrackingInteropService _interop = new();

    public ButtonPressEffectReconcileTests()
    {
    ...
    [Fact]
    public async Task Disposing_A_Ripple_Button_Detaches_The_Listener()
    {
        var cut = _ctx.Render<Lumeo.Button>(p => p
            .Add(b => b.PressEffect, Lumeo.Button.ButtonPressEffect.Ripple)
            .AddChildContent("Btn"));
        ...
        // Tearing the component down must release the listener (subsumes #22's leak).
