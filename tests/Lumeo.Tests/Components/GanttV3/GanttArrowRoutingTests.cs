...
///
/// Codex round 2 (P1 #3, sticky-header restructure): these numbers no longer add
/// GanttScale.HeaderHeight (56) — the header used to live INSIDE the same outer
/// canvas div GanttArrowLayer's SVG is positioned against (via normal document
/// flow), so that div's coordinate origin sat at the header's top rather than
/// row 0's top; moving the header OUTSIDE that div entirely (GanttTimeline.razor's
/// own remarks) removed the offset from BOTH sides at once — GanttArrowLayer's
/// origin and a rendered GanttBar's own RowIndex*RowHeight math now agree with no
/// adjustment needed. See <see cref="GanttArrowRouting.SourceEdge"/>/
/// <see cref="GanttArrowRouting.TargetEdge"/>'s own remarks for the before/after.
///
