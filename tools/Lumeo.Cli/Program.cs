...
// --- theme apply ---
var themePresetOpt = new Option<string?>("--preset", "Preset code from the /themes customizer (6 base62 chars, e.g. b4Ndd7).");
var themeDryOpt = new Option<bool>("--dry-run", "Print decoded preset without writing anything.");
var themeApplyCmd = new Command("apply", "Apply a theme preset code from the /themes customizer.")
{
    themePresetOpt, themeDryOpt,
};
themeApplyCmd.SetHandler(async ctx =>
{
    var p = ctx.ParseResult.GetValueForOption(themePresetOpt) ?? "";
    var d = ctx.ParseResult.GetValueForOption(themeDryOpt);
    await ThemeCommands.Apply(p, d);
});
var themeCmd = new Command("theme", "Apply theme presets generated by the /themes customizer.");
