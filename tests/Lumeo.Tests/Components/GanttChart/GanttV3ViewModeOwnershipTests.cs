...
/// <summary>
/// GanttChart's view-mode OWNERSHIP model (Codex PR-382 review round 3, P1
...
public class GanttV3ViewModeOwnershipTests : IAsyncLifetime
{
    ...
    private static async Task ForceRepaintAsync(IRenderedComponent<L.GanttChart> cut)
    {
    ...
    private static MethodInfo Method(string name) =>
        typeof(L.GanttChart).GetMethod(name, BindingFlags.NonPublic | BindingFlags.Instance)!;

    private static GanttViewModeIntent Intent(IRenderedComponent<L.GanttChart> cut) =>
        (GanttViewModeIntent)typeof(L.GanttChart)
    ...
    private static string Label(IRenderedComponent<L.GanttChart> cut) =>
    ...
    private static void AssertMonthMode(IRenderedComponent<L.GanttChart> cut, string because)
    {
        var label = Label(cut);
    ...
    // Renders a CONTROLLED GanttChart whose consumer handler records its calls and
    ...
    private IRenderedComponent<L.GanttChart> RenderControlled(
    ...
    //
    // Path A — the flagged one (GanttChart.razor:518): the toolbar's own reconcile
    ...
    private Task PushParentRenderAsync(
        IRenderedComponent<L.GanttChart> cut, List<L.GanttTask> tasks, L.GanttViewMode parentViewMode,
        ...
        cut.InvokeAsync(() => cut.Instance.SetParametersAsync(ParameterView.FromDictionary(new Dictionary<string, object?>
        {
            ...
            [nameof(L.GanttChart.ViewMode)] = parentViewMode,
            [nameof(L.GanttChart.ViewModeChanged)] = EventCallback.Factory.Create<L.GanttViewMode>(this, async mode =>
            {
                calls.Add(mode);
                await gate.Task;
            }),
    ...
    public async Task A_Quick_Parameter_Reversion_Supersedes_The_Reconcile_Whose_Intent_It_Replaced()
    {
        ...
        async Task<Task> PushAsync(L.GanttViewMode mode)
        {
            ...
            await cut.InvokeAsync(() =>
            {
                ...
                {
                    [nameof(L.GanttChart.Tasks)] = tasks,
                    [nameof(L.GanttChart.ViewMode)] = mode,
                    [nameof(L.GanttChart.ViewModeChanged)] = callback,
                    [nameof(L.GanttChart.ShowTreePane)] = false,
                }));
            });
            return pass;
    ...
    public async Task Adding_A_ViewModeChanged_Delegate_Later_Reconciles_To_The_Controlling_Parameter()
    {
        ...
        await cut.InvokeAsync(() => cut.Instance.SetParametersAsync(ParameterView.FromDictionary(new Dictionary<string, object?>
        {
            ...
            [nameof(L.GanttChart.ViewModeChanged)] = EventCallback.Factory.Create<L.GanttViewMode>(this, m => calls.Add(m)),
            [nameof(L.GanttChart.ShowTreePane)] = false,
        })));
        await ForceRepaintAsync(cut);
    ...
    private static int ScrollRequestId(IRenderedComponent<L.GanttChart> cut) =>
        (int)typeof(L.GanttChart)
            .GetField("_scrollToTodayRequestId", BindingFlags.NonPublic | BindingFlags.Instance)!
            .GetValue(cut.Instance)!;

    private static DateTime? ScrollTargetDate(IRenderedComponent<L.GanttChart> cut) =>
        (DateTime?)typeof(L.GanttChart)
    ...
    public async Task Removing_The_Controlled_Binding_While_Notifying_Does_Not_Revert_The_Pick()
    {
        ...
        await cut.InvokeAsync(() => cut.Instance.SetParametersAsync(ParameterView.FromDictionary(new Dictionary<string, object?>
        {
            [nameof(L.GanttChart.Tasks)] = tasks,
            [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Day,
            [nameof(L.GanttChart.ViewModeChanged)] = default(EventCallback<L.GanttViewMode>),
            [nameof(L.GanttChart.ShowTreePane)] = false,
        })));
    ...
    public async Task A_Newer_Parameter_Push_Supersedes_A_Toolbar_Pick_Whose_Callback_Is_Still_Outstanding()
    {
        ...
        {
            [nameof(L.GanttChart.Tasks)] = tasks,
            [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Year,
            [nameof(L.GanttChart.ViewModeChanged)] = EventCallback.Factory.Create<L.GanttViewMode>(this, m => calls.Add(m)),
            [nameof(L.GanttChart.ShowTreePane)] = false,
        })));
    ...
    public async Task Once_Everything_Quiesces_Exactly_One_Settled_Intent_Matches_The_Committed_Mode()
    {
        ...
        {
            [nameof(L.GanttChart.Tasks)] = tasks,
            [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Month, // what the settled intent told the parent to hold
            [nameof(L.GanttChart.ViewModeChanged)] = EventCallback.Factory.Create<L.GanttViewMode>(this, m => calls.Add(m)),
            [nameof(L.GanttChart.ShowTreePane)] = false,
        })));
        Assert.Equal(idBeforeNav, Intent(cut).Id);
