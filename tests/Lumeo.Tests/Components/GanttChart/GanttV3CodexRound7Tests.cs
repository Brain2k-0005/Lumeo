...
public class GanttV3CodexRound7Tests : IAsyncLifetime
{
    ...
    // ── GanttChart: browser-date staleness fix (P2 #3) ──────────────────────────

    [Fact]
    public void GanttChart_GoToTodayAsync_Always_Requeries_The_Browser_Date_On_An_Explicit_Today_Click()
    {
    ...
    // ── GanttChart: browser-date thrash fix (cx7b review, Important #2) ────────
    //
    // Superseded the original round-7 "GanttChart_Reresolves_The_Browser_Date_
    ...
    private static void SetLastCheckedServerDay(L.GanttChart instance, DateTime value)
    {
        var field = typeof(L.GanttChart).GetField("_lastCheckedServerDay", BindingFlags.NonPublic | BindingFlags.Instance);
        Assert.NotNull(field);
        field!.SetValue(instance, (DateTime?)value);
    }

    [Fact]
    public void GanttChart_Does_Not_Requery_The_Browser_On_Every_Render_While_The_Servers_Day_Is_Unchanged()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            // already "t1", and Task.Id must stay unique per GanttChart instance
            // (GanttBar's own @key relies on it; see GanttChartCodexRound15Tests'
    ...
    [Fact]
    public void GanttChart_Requeries_Exactly_Once_When_The_Servers_Own_Day_Has_Advanced_Since_The_Last_Check()
    {
