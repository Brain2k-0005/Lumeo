...
public class GanttV3CodexRound17Tests : IAsyncLifetime
{
    ...
    public async Task Finding1_An_Unrelated_Parameter_ReRender_Does_Not_Abort_A_Suspended_Reconcile()
    {
        ...
        await cut.InvokeAsync(() =>
        {
            reconcile = cut.Instance.SetParametersAsync(ParameterView.FromDictionary(new Dictionary<string, object?>
            {
                ...
                [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Week,
                [nameof(L.GanttChart.ShowTreePane)] = false,
                [nameof(L.GanttChart.Class)] = "initial-class",
            }));
        });
        ...
        await cut.InvokeAsync(async () =>
        {
            ...
            {
                [nameof(L.GanttChart.Tasks)] = tasks,
                [nameof(L.GanttChart.ViewMode)] = L.GanttViewMode.Week, // matches _state.ViewMode already (uncontrolled, unchanged since last pass)
                [nameof(L.GanttChart.ShowTreePane)] = false,
                [nameof(L.GanttChart.Class)] = "different-class",
            }));
        });
    ...
    [Fact]
    public void Finding5_A_Zero_ColumnWidth_Override_Also_Falls_Back_On_GanttCharts_Own_Reconcile_Facing_Property()
    {
        // GanttChart.EffectiveColumnWidthFor feeds the reconcile's own snapshot/
        ...
        var method = typeof(L.GanttChart).GetMethod(
