namespace Lumeo.Services.Localization;

internal static partial class LumeoDefaultStrings
{
    internal static readonly IReadOnlyDictionary<string, string> Es = new Dictionary<string, string>
    {
        // DataGrid
        ["DataGrid.NoData"] = "Sin datos",
        ["DataGrid.NoDataFiltered"] = "Ninguna fila coincide con los filtros actuales",
        ["DataGrid.Loading"] = "Cargando…",
        ["DataGrid.SearchPlaceholder"] = "Buscar…",
        ["DataGrid.ClearSearch"] = "Borrar búsqueda",
        ["DataGrid.Columns"] = "Columnas",
        ["DataGrid.ToggleColumns"] = "Mostrar/ocultar columnas",
        ["DataGrid.ExportCsv"] = "Exportar CSV",
        ["DataGrid.ExportExcel"] = "Exportar Excel",
        ["DataGrid.ExportJson"] = "Exportar JSON",
        ["DataGrid.Export"] = "Exportar",
        ["DataGrid.Filter"] = "Filtro",
        ["DataGrid.Filters"] = "Filtros",
        ["DataGrid.ClearFilters"] = "Limpiar filtros",
        ["DataGrid.ResetLayout"] = "Restablecer diseño",
        ["DataGrid.SaveLayout"] = "Guardar diseño",
        ["DataGrid.Layouts"] = "Diseños",
        ["DataGrid.NewLayout"] = "Nuevo diseño",
        ["DataGrid.Personal"] = "Personal",
        ["DataGrid.Global"] = "Global",
        ["DataGrid.SystemDefault"] = "Predeterminado del sistema",
        ["DataGrid.LayoutName"] = "Nombre del diseño",
        ["DataGrid.Save"] = "Guardar",
        ["DataGrid.Cancel"] = "Cancelar",
        ["DataGrid.Delete"] = "Eliminar",
        ["DataGrid.Rename"] = "Renombrar",
        ["DataGrid.PinLeft"] = "Fijar a la izquierda",
        ["DataGrid.PinRight"] = "Fijar a la derecha",
        ["DataGrid.Unpin"] = "Desanclar",
        ["DataGrid.PinColumn"] = "Fijar columna",
        ["DataGrid.ColumnMenu"] = "Menú de columna",
        ["DataGrid.SortAscending"] = "Ordenar ascendente",
        ["DataGrid.SortDescending"] = "Ordenar descendente",
        ["DataGrid.FitToContent"] = "Ajustar al contenido",
        ["DataGrid.AutoSizeAllColumns"] = "Ajustar automáticamente todas las columnas",
        ["DataGrid.MoveLeft"] = "Mover a la izquierda",
        ["DataGrid.MoveRight"] = "Mover a la derecha",
        ["DataGrid.ResizeColumn"] = "Cambiar tamaño de columna (use las flechas, doble clic para ajuste automático)",
        ["DataGrid.DragToReorder"] = "Arrastrar para reordenar la columna",
        ["DataGrid.ColumnMovedAnnouncement"] = "{0} movido a la posición {1} de {2}",
        ["DataGrid.FilterColumn"] = "Filtrar {0}",
        ["DataGrid.SelectRow"] = "Seleccionar fila {0}",
        ["DataGrid.SelectAllRows"] = "Seleccionar todas las filas",
        ["DataGrid.ExpandRow"] = "Expandir fila {0}",
        ["DataGrid.CollapseRow"] = "Contraer fila {0}",
        ["DataGrid.ExpandGroup"] = "Expandir grupo {0}",
        ["DataGrid.CollapseGroup"] = "Contraer grupo {0}",
        ["DataGrid.Hide"] = "Ocultar",
        ["DataGrid.Show"] = "Mostrar",
        ["DataGrid.SortAscending"] = "Orden ascendente",
        ["DataGrid.SortDescending"] = "Orden descendente",
        ["DataGrid.ClearSort"] = "Quitar orden",
        ["DataGrid.Edit"] = "Editar",
        ["DataGrid.CommitEdit"] = "Guardar",
        ["DataGrid.CancelEdit"] = "Cancelar",
        ["DataGrid.AggregateSum"] = "Suma",
        ["DataGrid.AggregateAvg"] = "Prom.",
        ["DataGrid.AggregateCount"] = "Total",
        ["DataGrid.AggregateRow"] = "Fila de agregación",
        ["DataGrid.AggregateMin"] = "Mín",
        ["DataGrid.AggregateMax"] = "Máx",
        ["DataGrid.Items"] = "elementos",
        ["DataGrid.ItemsCount"] = "{0} elementos",
        ["DataGrid.ItemsCount.One"] = "{0} elemento",
        ["DataGrid.ItemsCount.Other"] = "{0} elementos",
        ["DataGrid.CopySelected"] = "Copiar ({0})",
        ["DataGrid.ApplyLayout"] = "Aplicar diseño",
        ["DataGrid.NoSavedLayouts"] = "Aún no hay diseños guardados.",
        ["DataGrid.SaveCurrentLayout"] = "Guardar diseño actual…",
        ["DataGrid.Default"] = "Predeterminado",
        ["DataGrid.MoveUp"] = "Subir",
        ["DataGrid.MoveDown"] = "Bajar",
        ["DataGrid.Retry"] = "Reintentar",
        ["DataGrid.ErrorLoadingData"] = "Error al cargar los datos: {0}",
        ["DataGrid.ExpandFullscreen"] = "Expandir a pantalla completa",
        ["DataGrid.ExitFullscreen"] = "Salir de pantalla completa",
        ["DataGrid.AddGroupLevel"] = "+ Añadir nivel de agrupación",
        ["DataGrid.GroupPanelPlaceholder"] = "Arrastre aquí un encabezado de columna agrupable o use la lista desplegable",
        ["DataGrid.DragToGroup"] = "Arrastrar para agrupar por esta columna",
        ["DataGrid.RemoveGrouping"] = "Quitar agrupación",
        ["DataGrid.ClearAllGrouping"] = "Borrar toda la agrupación",
        ["DataGrid.DragToReorderRow"] = "Arrastrar para reordenar la fila",
        ["DataGrid.RowReorderUnavailable"] = "El reordenamiento de filas no está disponible durante la agrupación o la virtualización",
        ["Filter.FilterTitle"] = "Filtro: {0}",

        // Pagination
        ["Pagination.Previous"] = "Anterior",
        ["Pagination.Next"] = "Siguiente",
        ["Pagination.First"] = "Primera",
        ["Pagination.Last"] = "Última",
        ["Pagination.Page"] = "Página",
        ["Pagination.MorePages"] = "Más páginas",
        ["Pagination.GoToPage"] = "Ir a la página {0}",
        ["Pagination.RowsPerPage"] = "Filas por página",
        ["Pagination.RangeOfTotal"] = "{0}–{1} de {2}",

        // Filter operators
        ["Filter.Contains"] = "contiene",
        ["Filter.DoesNotContain"] = "no contiene",
        ["Filter.Equals"] = "igual a",
        ["Filter.NotEquals"] = "distinto de",
        ["Filter.StartsWith"] = "empieza con",
        ["Filter.EndsWith"] = "termina con",
        ["Filter.GreaterThan"] = "mayor que",
        ["Filter.LessThan"] = "menor que",
        ["Filter.GreaterThanOrEqual"] = "mayor o igual",
        ["Filter.LessThanOrEqual"] = "menor o igual",
        ["Filter.Between"] = "entre",
        ["Filter.IsEmpty"] = "está vacío",
        ["Filter.IsNotEmpty"] = "no está vacío",
        ["Filter.Apply"] = "Aplicar",
        ["Filter.Clear"] = "Limpiar",
        ["Filter.Value"] = "Valor",
        ["Filter.SelectAll"] = "Todos",
        ["Filter.ValuePlaceholder"] = "Valor…",
        ["Filter.ToValuePlaceholder"] = "Hasta…",

        // ColorPicker
        ["ColorPicker.PickColor"] = "Elegir un color",
        ["ColorPicker.Hue"] = "Tono",
        ["ColorPicker.Saturation"] = "Saturación",
        ["ColorPicker.Lightness"] = "Luminosidad",
        ["ColorPicker.Value"] = "Valor",
        ["ColorPicker.Opacity"] = "Opacidad",
        ["ColorPicker.Hex"] = "Hex",
        ["ColorPicker.HexValue"] = "Valor hex",
        ["ColorPicker.Red"] = "Rojo",
        ["ColorPicker.Green"] = "Verde",
        ["ColorPicker.Blue"] = "Azul",
        ["ColorPicker.Presets"] = "Preajustes",

        // ── IconPicker ────────────────────────────────
        ["IconPicker.Placeholder"] = "Elegir un icono",
        ["IconPicker.SearchPlaceholder"] = "Buscar iconos",
        ["IconPicker.NoIconsFound"] = "No se encontraron iconos",

        // PasswordInput
        ["Password.Placeholder"] = "Introducir contraseña",
        ["Password.Toggle"] = "Mostrar/ocultar contraseña",
        ["Password.Weak"] = "Débil",
        ["Password.Fair"] = "Aceptable",
        ["Password.Good"] = "Buena",
        ["Password.Strong"] = "Fuerte",

        // FileUpload
        ["FileUpload.DragDrop"] = "Arrastra y suelta archivos aquí",
        ["FileUpload.Or"] = "o",
        ["FileUpload.Browse"] = "Examinar",
        ["FileUpload.MaxSize"] = "Tamaño máx.: {0}",
        ["FileUpload.Accepted"] = "Aceptados: {0}",
        ["FileUpload.Remove"] = "Quitar",
        ["FileUpload.Uploading"] = "Subiendo…",
        ["FileUpload.Uploaded"] = "Subido",
        ["FileUpload.Failed"] = "Error",
        ["FileUpload.Retry"] = "Reintentar",
        ["FileUpload.TooLarge"] = "Archivo demasiado grande",
        ["FileUpload.TypeNotAllowed"] = "Tipo de archivo no permitido",
        ["FileUpload.ChooseFile"] = "Elegir archivo",
        ["FileUpload.ClickToUpload"] = "Haz clic para subir o arrastra aquí",

        // Overlays
        ["Dialog.Close"] = "Cerrar",
        ["Dialog.Confirm"] = "Confirmar",
        ["Dialog.Cancel"] = "Cancelar",
        ["Dialog.Ok"] = "Aceptar",
        ["Dialog.Yes"] = "Sí",
        ["Dialog.No"] = "No",
        ["Toast.Close"] = "Cerrar",
        ["Toast.Dismiss"] = "Descartar",
        ["AlertDialog.Delete"] = "Eliminar",
        ["AlertDialog.Continue"] = "Continuar",

        // Combobox / Command / Select
        ["Combobox.Placeholder"] = "Seleccionar…",
        ["Combobox.SearchPlaceholder"] = "Buscar…",
        ["Combobox.NoResults"] = "Sin resultados",
        ["Combobox.Loading"] = "Cargando…",
        ["Combobox.Clear"] = "Limpiar",
        ["Combobox.Create"] = "Crear \"{0}\"",
        ["Command.Placeholder"] = "Escribe un comando o busca…",
        ["Command.NoResults"] = "Sin resultados",
        ["Select.Placeholder"] = "Selecciona una opción",

        // Calendar / DatePicker
        ["Calendar.Today"] = "Hoy",
        ["Calendar.Clear"] = "Limpiar",
        ["Calendar.PrevMonth"] = "Mes anterior",
        ["Calendar.NextMonth"] = "Mes siguiente",
        ["Calendar.PrevYear"] = "Año anterior",
        ["Calendar.NextYear"] = "Año siguiente",
        ["DatePicker.Placeholder"] = "Elegir fecha",
        ["DateRange.Placeholder"] = "Elegir rango de fechas",
        ["DateRange.From"] = "Desde",
        ["DateRange.To"] = "Hasta",
        ["DateTimePicker.Placeholder"] = "Elegir fecha y hora",
        ["DateTimePicker.TimeLabel"] = "Hora",
        ["TimePicker.Placeholder"] = "Elegir hora",

        // Tour
        ["Tour.Skip"] = "Omitir",
        ["Tour.Previous"] = "Anterior",
        ["Tour.Next"] = "Siguiente",
        ["Tour.Finish"] = "Finalizar",

        // PopConfirm
        ["PopConfirm.Title"] = "¿Estás seguro?",
        ["PopConfirm.Confirm"] = "Sí",
        ["PopConfirm.Cancel"] = "No",

        // Misc
        ["Common.Search"] = "Buscar",
        ["Common.Clear"] = "Limpiar",
        ["Common.ClearAll"] = "Limpiar todo",
        ["Common.Close"] = "Cerrar",
        ["Common.Loading"] = "Cargando…",
        ["Common.NoResults"] = "Sin resultados",
        ["Common.Apply"] = "Aplicar",
        ["Common.Reset"] = "Restablecer",
        ["Common.Save"] = "Guardar",
        ["Common.Cancel"] = "Cancelar",
        ["Common.Copy"] = "Copiar",
        ["Common.Copied"] = "Copiado",
        ["Common.More"] = "Más",
        ["Common.Back"] = "Atrás",
        ["Common.Next"] = "Siguiente",
        ["Common.ShowMore"] = "Mostrar más",
        ["Common.ShowLess"] = "Mostrar menos",

        // Transfer / TreeSelect / TagInput
        ["Transfer.SourceHeader"] = "Disponibles",
        ["Transfer.TargetHeader"] = "Seleccionados",
        ["Transfer.MoveRight"] = "Mover a la derecha",
        ["Transfer.MoveLeft"] = "Mover a la izquierda",
        ["Transfer.NoItems"] = "Sin elementos",
        ["TreeSelect.Placeholder"] = "Seleccionar…",
        ["TreeSelect.NoResults"] = "Sin resultados",
        ["TagInput.Placeholder"] = "Añadir etiqueta…",

        // Cascader
        ["Cascader.Placeholder"] = "Seleccionar…",

        // Rating / OTP
        ["Rating.Rate"] = "Valorar",
        ["Rating.RateOf"] = "Valoración {0} de {1}",
        ["Otp.Placeholder"] = "Introducir código",

        // Empty
        ["Empty.Title"] = "Nada por aquí",
        ["Empty.Description"] = "No hay datos para mostrar.",

        // Kanban
        ["Kanban.AddCard"] = "Añadir tarjeta",

        // Carousel
        ["Carousel.PreviousSlide"] = "Diapositiva anterior",
        ["Carousel.NextSlide"] = "Siguiente diapositiva",
        ["Carousel.SlideXofY"] = "Diapositiva {0} de {1}",

        // Stepper
        ["Stepper.Back"] = "Atrás",
        ["Stepper.Next"] = "Siguiente",
        ["Stepper.Finish"] = "Finalizar",
        ["Stepper.Optional"] = "Opcional",
        ["Stepper.Skip"] = "Omitir",

        // Skeleton
        ["Skeleton.Loading"] = "Cargando",

        // Window
        ["Window.Close"] = "Cerrar",
        ["Window.Minimize"] = "Minimizar",
        ["Window.Maximize"] = "Maximizar",
        ["Window.Restore"] = "Restaurar",

        // NumberInput
        ["NumberInput.Decrease"] = "Disminuir",
        ["NumberInput.Increase"] = "Aumentar",

        // DateTimePicker
        ["DateTimePicker.ClearDate"] = "Borrar fecha",

        // Slider
        ["Slider.End"] = "final",

        // FileManager
        ["FileManager.EmptyTitle"] = "Esta carpeta está vacía",
        ["FileManager.EmptyState"] = "No hay archivos ni carpetas aquí.",
        ["FileManager.MoreActions"] = "Más acciones",
        ["FileManager.MoreActionsForName"] = "Más acciones para {0}",

        // FileUpload (parameterised)
        ["FileUpload.ExceedsMaxSize"] = "El archivo «{0}» supera el tamaño máximo de {1}.",

        // ConfirmButton
        ["ConfirmButton.Title"] = "¿Estás seguro?",
        ["ConfirmButton.Confirm"] = "Continuar",
        ["ConfirmButton.Cancel"] = "Cancelar",

        // PickList (panels)
        ["PickList.SourceHeader"] = "Disponibles",
        ["PickList.TargetHeader"] = "Seleccionados",
        ["PickList.NoItems"] = "Sin elementos",

        // FileManager (context menu / list view)
        ["FileManager.Open"] = "Abrir",
        ["FileManager.Rename"] = "Renombrar",
        ["FileManager.Delete"] = "Eliminar",
        ["FileManager.Name"] = "Nombre",
        ["FileManager.Size"] = "Tamaño",
        ["FileManager.Modified"] = "Modificado",
        ["FileManager.Loading"] = "Cargando…",

        // AudioPlayer
        ["AudioPlayer.Label"] = "Reproductor de audio",
        ["AudioPlayer.Play"] = "Reproducir",
        ["AudioPlayer.Pause"] = "Pausar",
        ["AudioPlayer.Seek"] = "Posición de reproducción",
        ["AudioPlayer.Mute"] = "Silenciar",
        ["AudioPlayer.Unmute"] = "Activar sonido",
        ["AudioPlayer.Download"] = "Descargar audio",

        // ThemeSwitcher
        ["Theme.Color"] = "Color",
        ["Theme.Mode"] = "Modo",

        // Scheduler (backfilled — previously only en/de/ar/zh/ko/ru/tr had any Scheduler.* keys)
        ["Scheduler.Previous"] = "Anterior",
        ["Scheduler.Next"] = "Siguiente",
        ["Scheduler.ResourceLegend"] = "Leyenda de recursos",
        ["Scheduler.Day"] = "Día",
        ["Scheduler.Week"] = "Semana",
        ["Scheduler.Month"] = "Mes",
        ["Scheduler.WeekNumberAbbrev"] = "Sem",
        ["Scheduler.WeekNumberLabel"] = "Semana {0}",
        ["Scheduler.ResourceGridLabel"] = "Recursos el {0}",
        ["Scheduler.Resources"] = "Recursos",
        ["Scheduler.Timeline"] = "Cronología",
        ["Scheduler.Calendars"] = "Calendarios",
        ["Scheduler.NewEvent"] = "Nuevo evento",
        ["Scheduler.EditEvent"] = "Editar evento",
        ["Scheduler.EventTitle"] = "Título",
        ["Scheduler.Starts"] = "Comienza",
        ["Scheduler.Ends"] = "Termina",
        ["Scheduler.AllDay"] = "Todo el día",
        ["Scheduler.NoCalendar"] = "Sin calendario",
        ["Scheduler.Save"] = "Guardar",
        ["Scheduler.Cancel"] = "Cancelar",
        ["Scheduler.Delete"] = "Eliminar",
        ["Scheduler.MergePanes"] = "Combinar paneles",
        ["Scheduler.SplitPanes"] = "En paralelo",
        ["Scheduler.ToggleCalendar"] = "Mostrar u ocultar {0}",
        ["Scheduler.NDays"] = "{0} días",
        ["Scheduler.Announce.EventMoved"] = "{0} movido al {1}",
        ["Scheduler.Announce.EventRescheduled"] = "{0} ahora de {1} a {2}",
        ["Scheduler.Announce.MoveRejected"] = "No se pudo mover {0}",
        ["Scheduler.Announce.ResizeRejected"] = "No se pudo cambiar la duración de {0}",
        ["Scheduler.List"] = "Lista",
        ["Scheduler.Today"] = "Hoy",
        ["Scheduler.WeekOf"] = "Semana del {0}",

        ["Scheduler.NoEvents"] = "Sin eventos",
        ["Scheduler.MoreEvents"] = "{0} más",
        ["Scheduler.MoreEvents.One"] = "{0} más",
        ["Scheduler.MoreEvents.Other"] = "{0} más",
        // Gantt (Codex round 4, P2 #6 — locale completeness: previously only en/de had any Gantt.* keys)
        ["Gantt.Day"] = "Día",
        ["Gantt.Week"] = "Semana",
        ["Gantt.Month"] = "Mes",
        ["Gantt.Year"] = "Año",
        ["Gantt.Quarter"] = "Trimestre",
        ["Gantt.Today"] = "Hoy",
        ["Gantt.PreviousPeriod"] = "Período anterior",
        ["Gantt.NextPeriod"] = "Período siguiente",
        ["Gantt.ExpandRow"] = "Expandir {0}",
        ["Gantt.CollapseRow"] = "Contraer {0}",
        ["Gantt.NewTask"] = "Nueva tarea",
        ["Gantt.NoTasksToDisplay"] = "No hay tareas que mostrar",
        ["Gantt.TaskAriaLabel"] = "{0}, del {1} al {2}",
        ["Gantt.PercentComplete"] = "{0}% completado",
        ["Gantt.SummaryProgress"] = "{0}%",
        ["Gantt.ResizeTreePane"] = "Redimensionar el panel de árbol",
        ["Gantt.SelectRow"] = "Seleccionar {0}",
        ["Gantt.SelectRowGroup"] = "Seleccionar todo en {0}",
        ["Gantt.DragToReorderRow"] = "Arrastrar para reordenar {0}",
        ["Gantt.RowReorderUnavailable"] = "No se puede reordenar filas en modo de solo lectura",
        ["Gantt.ZoomIn"] = "Acercar",
        ["Gantt.ZoomOut"] = "Alejar",
        ["Flow.CanvasLabel"] = "Lienzo de flujo",
        ["Flow.NodeRole"] = "nodo",
        ["Flow.Controls"] = "Controles del lienzo",
        ["Flow.ZoomIn"] = "Acercar",
        ["Flow.ZoomOut"] = "Alejar",
        ["Flow.FitView"] = "Ajustar vista",
        ["Flow.Lock"] = "Bloquear lienzo",
        ["Flow.ConnectFrom"] = "Conectar desde {0}",
        ["Flow.ConnectTo"] = "Conectar con {0}",
        ["Flow.MiniMap"] = "Vista general",
        ["Flow.NodeToolbar"] = "Acciones del nodo",
        ["Flow.SnapToGrid"] = "Ajustar a la cuadrícula",
        ["Flow.Undo"] = "Deshacer",
        ["Flow.Redo"] = "Rehacer",
        ["Flow.ReconnectHandle"] = "Mover extremo de la arista",
        ["Flow.ConnectingFrom"] = "Conectando desde {0}. Pulsa Intro en un controlador de destino para conectar, Escape para cancelar.",
        ["Flow.Connected"] = "Conectado.",
        ["Flow.ConnectionRejected"] = "Conexión rechazada.",
        ["Flow.ConnectionCancelled"] = "Conexión cancelada.",
        ["Flow.Reconnected"] = "Arista reconectada.",
        ["Flow.ReconnectRejected"] = "Reconexión rechazada.",
        ["Flow.EdgeLabelEdit"] = "Editar etiqueta de la conexión",
        ["Flow.EdgeLabelUpdated"] = "Etiqueta actualizada.",
        ["Flow.GroupDefaultLabel"] = "Grupo",
        ["Flow.GroupRole"] = "grupo",
        ["Flow.GroupChildCountOne"] = "1 nodo",
        ["Flow.GroupChildCount"] = "{0} nodos",
        ["Flow.Grouped"] = "{0} nodos agrupados.",
        ["Flow.Ungrouped"] = "{0} nodos desagrupados.",
        ["Gantt.ScrollToEarlierTask"] = "Desplazar a {0} (antes)",
        ["Gantt.ScrollToLaterTask"] = "Desplazar a {0} (después)",
        ["Gantt.Settings"] = "Configuración",
        ["Gantt.SettingsDisplay"] = "Visualización",
        ["Gantt.SettingsBehavior"] = "Comportamiento",
        ["Gantt.SettingsRegion"] = "Región",
        ["Gantt.SettingsStyle"] = "Estilo",
        ["Gantt.SettingsShowRowCheckboxes"] = "Casillas de fila",
        ["Gantt.SettingsShowSummaryBars"] = "Barras de resumen",
        ["Gantt.SettingsShowZoomControl"] = "Control de zoom",
        ["Gantt.SettingsShowOffscreenIndicators"] = "Indicadores fuera de pantalla",
        ["Gantt.SettingsInfiniteScroll"] = "Desplazamiento infinito",
        ["Gantt.SettingsNowIndicator"] = "Indicador de ahora",
        ["Gantt.SettingsMarkOffDays"] = "Marcar días libres",
        ["Gantt.SettingsReadonly"] = "Solo lectura",
        ["Gantt.SettingsAllowCreate"] = "Permitir creación",
        ["Gantt.SettingsAllowRowReorder"] = "Permitir reordenar filas",
        ["Gantt.SettingsFirstDayOfWeek"] = "Primer día de la semana",
        ["Gantt.SettingsOffDays"] = "Días libres",
        ["Gantt.SettingsColorByGroup"] = "Color por grupo",
        ["Gantt.SettingsReset"] = "Restablecer valores predeterminados",
        // Filters
        ["Filters.AddFilter"] = "Añadir filtro",
        ["Filters.AdvancedFilter"] = "Filtro avanzado",
        ["Filters.ShowRecords"] = "En esta vista, mostrar registros",
        ["Filters.BuilderEmpty"] = "Aún no hay filtros",
        ["Filters.BuilderEmptyHint"] = "Añade un filtro para acotar lo que ves.",
        ["Filters.AddCondition"] = "Añadir filtro",
        ["Filters.AddGroup"] = "Añadir grupo",
        ["Filters.AddToGroup"] = "Añadir filtro a este grupo",
        ["Filters.RemoveGroup"] = "Quitar grupo",
        ["Filters.WrapInGroup"] = "Agrupar",
        ["Filters.Ungroup"] = "Desagrupar",
        ["Filters.MoveToTop"] = "Mover arriba",
        ["Filters.MoveToGroup"] = "Mover al grupo {0}",
        ["Filters.Reorder"] = "Reordenar",
        ["Filters.ReorderHint"] = "Pulsa Alt con Flecha arriba o abajo para reordenar",
        ["Filters.GroupAll"] = "Todas las condiciones siguientes se cumplen...",
        ["Filters.GroupAny"] = "Alguna de las condiciones siguientes se cumple...",
        ["Filters.GroupPlaceholder"] = "Arrastra los filtros aquí",
        ["Filters.GroupAdded"] = "Grupo añadido",
        ["Filters.GroupRemoved"] = "Grupo quitado",
        ["Filters.ReorderAnnouncement"] = "{0} movido a la posición {1} de {2}",
        ["Filters.MoveAnnouncement"] = "{0} movido a {1}, posición {2} de {3}",
        ["Filters.ClearAll"] = "Borrar todo",
        ["Filters.GroupMenu"] = "Opciones del grupo",
        ["Filters.SearchFields"] = "Buscar atributos...",
        ["Filters.SearchOperators"] = "Buscar condiciones...",
        ["Filters.SearchOptions"] = "Buscar...",
        ["Filters.Back"] = "Atrás",
        ["Filters.Clear"] = "Borrar",
        ["Filters.Apply"] = "Aplicar",
        ["Filters.Discard"] = "Descartar cambios",
        ["Filters.Empty"] = "Sin resultados",
        ["Filters.Loading"] = "Cargando...",
        ["Filters.LoadingMore"] = "Cargando más...",
        ["Filters.LoadMore"] = "Cargar más",
        ["Filters.Error"] = "No se pudo cargar",
        ["Filters.Retry"] = "Reintentar",
        ["Filters.Where"] = "Donde",
        ["Filters.And"] = "Y",
        ["Filters.Or"] = "O",
        ["Filters.Combinator"] = "Cambiar combinación",
        ["Filters.CombinatorLabel"] = "{0}, cambiar combinación",
        ["Filters.Duplicate"] = "Duplicar",
        ["Filters.Negate"] = "Negar",
        ["Filters.ConvertToAdvanced"] = "Editor avanzado",
        ["Filters.Remove"] = "Quitar",
        ["Filters.ChipMenu"] = "Opciones del filtro {0}",
        ["Filters.FiltersLabel"] = "Filtros",
        ["Filters.ReadOnly"] = "Solo lectura. Estos filtros no se pueden cambiar.",
        ["Filters.PathSeparator"] = " > ",
        ["Filters.ValuePlaceholder"] = "escribe un texto...",
        ["Filters.SelectPlaceholder"] = "Seleccionar...",
        ["Filters.NoValue"] = "sin valor",
        ["Filters.SelectCondition"] = "Elegir condición",
        ["Filters.Incomplete"] = "filtro incompleto",
        ["Filters.BranchAffordance"] = "abre una lista",
        ["Filters.ExclusiveHint"] = "no se puede combinar con las otras opciones",
        ["Filters.ExclusiveAnnouncement"] = "{0} seleccionado. {1} otra(s) selección(es) borrada(s).",
        ["Filters.ItemCount"] = "{0} elementos",
        ["Filters.FieldsLabel"] = "Atributos",
        ["Filters.ResultsAnnouncement"] = "{0} resultados",
        ["Filters.ActionsLabel"] = "Acciones",
        ["Filters.StepField"] = "Elige un atributo. {0}",
        ["Filters.StepOperator"] = "Elige una condición para {0}",
        ["Filters.StepValue"] = "Introduce un valor para {0}",
        ["Filters.CountOne"] = "1 filtro aplicado",
        ["Filters.CountOther"] = "{0} filtros aplicados",
        ["Filters.ValueCount"] = "{0} seleccionados",
        ["Filters.ValueDetail"] = "{0}: {1}",
        ["Filters.ValueRange"] = "{0} a {1}",
        ["Filters.RangeFrom"] = "{0} desde",
        ["Filters.RangeTo"] = "{0} hasta",
        ["Filters.RangeSeparator"] = "a",
        ["Filters.Negated"] = "no {0}",
        ["Filters.IssueOperator"] = "Elige una condición",
        ["Filters.IssueValue"] = "Introduce un valor",
        ["Filters.IssueRange"] = "Introduce ambos extremos del rango",
        ["Filters.IssueRangeOrder"] = "El final del rango es anterior a su inicio",
        ["Filters.IssueEmptyGroup"] = "Este grupo aún no tiene condiciones",
        ["Filters.IssueSummary"] = "{0} filas necesitan atención",
["Filters.RowLevel"] = "{0}, nivel {1}",
        ["Filters.True"] = "Verdadero",
        ["Filters.False"] = "Falso",
        ["Filters.DateToday"] = "hoy",
        ["Filters.DateTomorrow"] = "mañana",
        ["Filters.DateYesterday"] = "ayer",
        ["Filters.DateInFormat"] = "en {0} {1}",
        ["Filters.DateAgoFormat"] = "hace {0} {1}",
        ["Filters.DateThisFormat"] = "este {0}",
        ["Filters.DateDay"] = "día",
        ["Filters.DateDays"] = "días",
        ["Filters.DateWeek"] = "semana",
        ["Filters.DateWeeks"] = "semanas",
        ["Filters.DateMonth"] = "mes",
        ["Filters.DateMonths"] = "meses",
        ["Filters.DateYear"] = "año",
        ["Filters.DateYears"] = "años",
        ["Filters.DateFrom"] = "Desde",
        ["Filters.DateTo"] = "Hasta",
        ["Filters.DatePlaceholder"] = "hoy, en 1 semana, 01/05/2024...",
        ["Filters.Op.contains"] = "contiene",
        ["Filters.Op.not_contains"] = "no contiene",
        ["Filters.Op.starts_with"] = "empieza por",
        ["Filters.Op.ends_with"] = "termina en",
        ["Filters.Op.is"] = "es",
        ["Filters.Op.is_not"] = "no es",
        ["Filters.Op.is_any_of"] = "es alguno de",
        ["Filters.Op.is_none_of"] = "no es ninguno de",
        ["Filters.Op.has_any_of"] = "tiene alguno de",
        ["Filters.Op.has_all_of"] = "tiene todos",
        ["Filters.Op.has_none_of"] = "no tiene ninguno de",
        ["Filters.Op.eq"] = "es igual a",
        ["Filters.Op.neq"] = "no es igual a",
        ["Filters.Op.gt"] = "es mayor que",
        ["Filters.Op.gte"] = "es mayor o igual que",
        ["Filters.Op.lt"] = "es menor que",
        ["Filters.Op.lte"] = "es menor o igual que",
        ["Filters.Op.between"] = "está entre",
        ["Filters.Op.not_between"] = "no está entre",
        ["Filters.Op.is_before"] = "es anterior a",
        ["Filters.Op.is_after"] = "es posterior a",
        ["Filters.Op.is_on_or_before"] = "es el o anterior a",
        ["Filters.Op.is_on_or_after"] = "es el o posterior a",
        ["Filters.Op.empty"] = "está vacío",
        ["Filters.Op.not_empty"] = "no está vacío",

        // QueryBuilder
        ["QueryBuilder.SelectField"] = "Seleccionar campo…",
        ["QueryBuilder.SelectPlaceholder"] = "Seleccionar…",
    };
}
