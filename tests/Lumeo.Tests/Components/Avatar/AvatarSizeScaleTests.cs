...
public class AvatarSizeScaleTests : IAsyncLifetime
{
    ...
    [Theory]
    [InlineData(L.Size.Xxs, "h-5", "w-5")]
    [InlineData(L.Size.Xs, "h-6", "w-6")]
    [InlineData(L.Size.Sm, "h-8", "w-8")]
    [InlineData(L.Size.Md, "h-10", "w-10")]
    [InlineData(L.Size.Lg, "h-12", "w-12")]
    [InlineData(L.Size.Xl, "h-16", "w-16")]
    [InlineData(L.Size.Xxl, "h-20", "w-20")]
    ...
    public void Xs_No_Longer_Falls_Through_To_Md()
    {
        ...
        Assert.Contains("h-6", xsTokens);
        Assert.DoesNotContain("h-10", xsTokens);
        Assert.Contains("h-10", mdTokens);
    }
    ...
    public void Xs_Is_Smaller_Than_Sm_Monotonic_Order_Restored()
    {
        ...
        Assert.Contains("h-6", xs.Find("div").GetAttribute("class")!.Split(' '));
        Assert.Contains("h-8", sm.Find("div").GetAttribute("class")!.Split(' '));
    }
}
