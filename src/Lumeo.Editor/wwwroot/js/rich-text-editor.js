// Lumeo RichTextEditor — TipTap v2 wrapper with Notion-tier features.
//
// Loaded on demand. All TipTap/ProseMirror/lowlight/highlight.js modules are
// vendored as a single self-hosted ESM bundle at
//   /_content/Lumeo.Editor/js/tiptap-bundle.js
// regenerated by `npm run build:tiptap` in src/Lumeo.Editor (esbuild). This
// keeps the editor CSP-compliant (no esm.sh runtime fetch, no `unsafe-eval`)
// and lets it run fully offline. Bundle is checked in so consumers cloning
// from source don't need npm to build.
//
// Includes: starter-kit, mention, table, code-block-lowlight, image, link,
// placeholder, typography, task list/item, plus a custom slash-command
// suggestion plugin, drag-handle plugin, bubble menu plugin, and smart paste
// handling (Word cleanup, autolink, basic Markdown).
//
// Override mechanism: consumers can set `window.lumeoTiptapBundleUrl` BEFORE
// the first RichTextEditor instance loads to point at a different bundle URL
// (e.g. a CDN-hosted copy or a custom build with extra extensions).
//
...
const DEFAULT_BUNDLE_URL = '/_content/Lumeo.Editor/js/tiptap-bundle.js';
...
async function loadTiptap() {
    ...
    _tiptapPromise = (async () => {
        const url = (typeof window !== 'undefined' && window.lumeoTiptapBundleUrl)
            ? window.lumeoTiptapBundleUrl
            : DEFAULT_BUNDLE_URL;
        // Single dynamic import resolves to the full TipTap stack; the bundle
        // exports each upstream module as a namespace (e.g. `core`, `pmState`)
        // and each highlight.js language as a default-named export.
        ...
        switch (name) {
            ...
            case 'pickWord': {
                ...
                inp.onchange = async () => {
                    ...
                    // Pre-flight size guard — refuse to even read the file into memory if
                    // it exceeds the configured limit. Default keeps a generous 8 MB ceiling.
                    const maxBytes = (entry.options && typeof entry.options.maxWordImportBytes === 'number')
                        ? entry.options.maxWordImportBytes
                        : (8 * 1024 * 1024);
                    if (f.size > maxBytes) {
                        try {
                            await entry.dotNetRef.invokeMethodAsync(
                                ...
                                f.size,
                                maxBytes);
                        } catch (e) { console.error('Word import size-error report failed:', e); }
                        return;
                    }
                    try {
                        ...
                        try { await entry.dotNetRef.invokeMethodAsync('OnWordImportFailed', 'read_error', f.size, maxBytes); } catch (_) {}
                    }
                };
                inp.click();
                break;
