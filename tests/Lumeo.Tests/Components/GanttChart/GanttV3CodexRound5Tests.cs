...
public class GanttV3CodexRound5Tests : IAsyncLifetime
{
    ...
    // ── GanttChart: RTL scroll-offset side (P1 #4) ──────────────────────────────

    [Fact]
    public void GanttChart_Does_Not_Add_The_Tree_Width_Offset_To_The_Scroll_Target_Under_Rtl()
    {
        // Bug fix (Codex round 5, P1 #4): under RTL, the outer flex row
        // wrapping GanttTree + GanttTimeline reverses its children's VISUAL
        // order (see GanttChart.ScrollHostLeadingOffset's own remarks), so
        ...
        ltrCtx.Render<L.GanttChart>(p => p.Add(c => c.Tasks, tasks).Add(c => c.ViewMode, L.GanttViewMode.Day).Add(c => c.ShowTreePane, true));
        ...
        rtlCtx.Render<L.DirectionProvider>(p => p
            ...
            .AddChildContent<L.GanttChart>(g => g.Add(c => c.Tasks, tasks).Add(c => c.ViewMode, L.GanttViewMode.Day).Add(c => c.ShowTreePane, true)));
    ...
    // ── GanttChart: view-mode switch reads the live scroll center (P2 #5) ──────

    [Fact]
    public void GanttChart_ViewMode_Switch_Reads_The_Live_Scroll_Center_When_Available()
    {
        ...
        // VisibleRange change at all. GanttChart now reads the pane's ACTUAL
