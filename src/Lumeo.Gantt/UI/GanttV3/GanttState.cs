...
/// precedent applied to a different forcing mechanism. <see
/// cref="EditorBrowsableAttribute"/>(Never) + DO-NOT-PROMOTE (ledger/memory-tracked,
/// no comment in PublicAPI.Unshipped.txt itself — established precedent), same as
/// every other GanttV3-namespaced type this campaign.
/// </summary>
...
/// its own — see <c>Gantt3</c>'s own remarks) and calls <see cref="GanttState.Attach"/>
...
internal interface IGanttStateHost
{
    ...
    /// mutation — see <c>Gantt3.ApplyViewModeIntentAsync</c>'s own remarks for why this
...
///
/// <c>public</c> (design spec Phase 3, T4): promoted the moment <c>Gantt3</c> exposes a
/// <c>State</c> parameter wired to it, per the phase-1 sequencing note this type's OWN
/// remarks originally flagged. <see cref="EditorBrowsableAttribute"/>(Never) until the
/// Phase-4 rename (this type keeps its Phase 1-3 working name until then, same as every
/// other GanttV3 type/component — see the Phase-3 ledger watch-list: "GanttState-Hoisting
/// (public State-Param) mit Phase-4-Rename sequenzieren"); DO-NOT-PROMOTE, ledger/memory
/// tracked like every other GanttV3 PublicAPI entry.
///
...
/// <c>internal</c> — <c>Gantt3</c>'s own commit machinery is the only legitimate direct
...
public sealed class GanttState
{
    ...
    // than whichever Gantt3 currently renders against it — the entire point of
    ...
    /// parent/descendant selection semantics belong to <c>Gantt3</c>'s own row-click
    ...
    /// write side of state stays <c>Gantt3</c>-only.
    /// </summary>
    ...
    /// detect a task-set change WITHOUT committing it yet — <c>Gantt3</c>'s viewport
    ...
    /// write <c>Gantt3</c>'s own reconcile-commit phase uses once its CLAIM/reconcile
    ...
    // `State` at mount only — see Gantt3's own remarks for why re-binding `State`
    ...
    /// by routing through its EXISTING scroll-intent interop (<c>Gantt3</c>'s own
    ...
    /// span" convention <c>Gantt3.VisibleCenterDate</c> already applies to the whole
    ...
    /// <c>Gantt3.ApplyViewModeIntentAsync</c>'s own remarks for why a direct write
    ...
    /// dispatch-and-forget precedent elsewhere in this codebase (<c>Gantt3.OnThemeChanged</c>):
