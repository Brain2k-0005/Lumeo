...
public class GanttV3Phase3T5Tests : IAsyncLifetime
{
    ...
    private IRenderedComponent<L.GanttChart> RenderTree(Action<Bunit.ComponentParameterCollectionBuilder<L.GanttChart>> configure) =>
        _ctx.Render<L.GanttChart>(p =>
        {
    ...
    public void A_Controlled_TreePaneWidth_Parameter_Value_Outside_The_Bounds_Is_Also_Clamped_On_Render()
    {
        ...
        // own JSInvokable-boundary clamp): a controlled caller can hand GanttChart an
        // out-of-range TreePaneWidth PARAMETER directly, with no drag involved at
        // all — GanttChart.EffectiveTreePaneWidth clamps that too, so the rendered
    ...
    // GanttTree.CommitSplitterWidth's OWN clamp, isolated from GanttChart entirely
    ...
    // GanttChart) pipeline clamps; a first attempt at disable-checking THIS layer
    // specifically found that GanttChart.HandleTreePaneResizeAsync's own redundant
    ...
    // ONLY GanttTree with no GanttChart in the render tree to rescue a broken clamp).
    [Fact]
    public async Task GanttTree_CommitSplitterWidth_Clamps_Below_Min_In_Isolation_From_GanttChart()
    {
        double? resized = null;
    ...
    [Fact]
    public async Task GanttTree_CommitSplitterWidth_Clamps_Above_Max_In_Isolation_From_GanttChart()
    {
        double? resized = null;
        var cut = _ctx.Render<L.GanttTree>(p => p
            .Add(c => c.Rows, RowsFor(Fixture().ToArray()))
    ...
    {
        // Bug found during T5's own disable-check pass (see GanttChart.HandleTreePaneResizeAsync's
    ...
    public async Task ScrollToDateAsync_After_A_Resize_Targets_A_Pixel_Consistent_With_The_New_Width()
    {
        ...
        // GanttChart.ScrollHostLeadingOffset live — see its own remarks).
