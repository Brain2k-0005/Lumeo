...
/// <summary>
/// Triage #57 + #58 (medium, state-on-data-change) — when the Tour's step dialog
/// stops being rendered, the per-step UI state must be dropped so a later
/// refill / reopen re-establishes it from scratch.
///
/// #57 "Focus is not moved back into the dialog after Steps goes empty-then-refills
/// (async load flicker) at the same step": <c>_focusedStep</c> was only reset in
/// <c>Close()</c>. When <c>Steps</c> emptied during an async load (dialog unrenders)
/// and then refilled at the SAME <c>CurrentStep</c>, <c>_focusedStep == CurrentStep</c>
/// still held, so <c>OnAfterRenderAsync</c> skipped the focus call and the keyboard
/// user was stranded behind the overlay.
///
/// #58 "Parent-driven dismiss (Open=false set externally) leaves _focusedStep /
/// _targetRect / _currentTargetSelector stale, breaking the next reopen":
/// <c>Close()</c> was the only place that reset those fields. A parent that set
/// <c>Open=false</c> directly (never calling <c>Close()</c>) left
/// <c>_currentTargetSelector</c> pointing at the old step, so on reopen
/// <c>UpdateTargetRect</c> early-returned (selector unchanged) and the spotlight
/// rect was never refetched.
///
/// Fix: in <c>OnParametersSetAsync</c>'s else branch (dialog not rendered), reset
/// <c>_focusedStep = -1; _currentTargetSelector = null; _targetRect = null;</c>,
/// mirroring <c>Close()</c>.
///
/// The testable seams are the recorded <c>FocusElement</c> and
/// <c>GetElementRectBySelector</c> interop calls (bUnit can't assert real DOM
/// focus or drive the JS scroller).
/// </summary>
public class TourStateResetOnDismissTests : IAsyncLifetime
{
    private readonly BunitContext _ctx = new();
    private readonly RecordingTourInterop _interop = new();

    public TourStateResetOnDismissTests()
    {
    ...
    /// <summary>
    /// #57 — Steps emptied (async load flicker) then refilled at the SAME step must
    /// re-focus the dialog. Without the fix _focusedStep stayed == CurrentStep across
    /// the empty render, so the second focus call never happened.
    /// </summary>
    [Fact]
    public void Refill_After_Empty_Flicker_Refocuses_The_Dialog()
    {
        ...
        // Opening focuses the dialog once.
        cut.WaitForAssertion(() =>
            Assert.Contains(_interop.FocusElementCalls, id => id.StartsWith("tour-tooltip-")));
        var focusCountAfterOpen = _interop.FocusElementCalls.Count(id => id.StartsWith("tour-tooltip-"));

        // Async load flicker: Steps go empty -> dialog unrenders (no role=dialog).
        cut.Render(p => p.Add(c => c.Steps, new List<L.Tour.TourStepConfig>()));
        Assert.Empty(cut.FindAll("[role='dialog']"));

        // Steps refill at the SAME CurrentStep -> dialog remounts and focus must
        // be moved back into it.
        cut.Render(p => p.Add(c => c.Steps, Steps()));

        cut.WaitForAssertion(() =>
        {
            var focusCountAfterRefill = _interop.FocusElementCalls.Count(id => id.StartsWith("tour-tooltip-"));
            Assert.True(focusCountAfterRefill > focusCountAfterOpen,
                $"Dialog was not re-focused after empty->refill flicker (focus calls: {focusCountAfterOpen} -> {focusCountAfterRefill}).");
        });
    }

    /// <summary>
    /// #58 — Parent sets Open=false directly (not via Close()), then reopens at the
    /// same step. The spotlight rect for that step's selector must be refetched.
    /// Without the fix _currentTargetSelector stayed set, so UpdateTargetRect
    /// early-returned on reopen and GetElementRectBySelector was never called again.
    /// </summary>
    [Fact]
    public void Reopen_After_Parent_Driven_Dismiss_Refetches_The_Target_Rect()
    {
        var cut = _ctx.Render<L.Tour>(p => p
            .Add(c => c.Steps, Steps())
            .Add(c => c.Open, true)
            .Add(c => c.CurrentStep, 1)); // selector "#step-two"

        cut.WaitForAssertion(() =>
            Assert.Contains("#step-two", _interop.GetElementRectBySelectorCalls));
        var rectCountAfterOpen = _interop.GetElementRectBySelectorCalls.Count(s => s == "#step-two");

        // Parent dismisses by setting Open=false externally (never calls Close()).
        cut.Render(p => p.Add(c => c.Open, false));
        Assert.Empty(cut.FindAll("[role='dialog']"));

        // Reopen at the SAME step. The target rect must be refetched.
        cut.Render(p => p.Add(c => c.Open, true));

        cut.WaitForAssertion(() =>
        {
            var rectCountAfterReopen = _interop.GetElementRectBySelectorCalls.Count(s => s == "#step-two");
            Assert.True(rectCountAfterReopen > rectCountAfterOpen,
                $"Target rect was not refetched after parent-driven dismiss + reopen (rect fetches for #step-two: {rectCountAfterOpen} -> {rectCountAfterReopen}).");
        });
    }

    /// <summary>
    /// Records the seams this test asserts (<c>FocusElement</c> and
    /// <c>GetElementRectBySelector</c>) and returns a non-null rect so the
    /// spotlight/move path runs. Every other member forwards to an inner
    /// <see cref="TrackingInteropService"/>, mirroring RecordingTourInterop in
    /// TourScrollIntoViewTests.
    /// </summary>
    private sealed class RecordingTourInterop : IComponentInteropService
    {
        ...
        private readonly List<string> _getElementRectBySelectorCalls = new();

        public IReadOnlyList<string> FocusElementCalls => _focusElementCalls;
        public IReadOnlyList<string> GetElementRectBySelectorCalls => _getElementRectBySelectorCalls;
        ...
        // Return a stable non-null rect so the spotlight has a target and the move
        // path (movingToNewTarget -> scroll -> lock -> UpdateTargetRect) runs.
        public ValueTask<ElementRect?> GetElementRectBySelector(string selector)
        {
            _getElementRectBySelectorCalls.Add(selector);
            return ValueTask.FromResult<ElementRect?>(new ElementRect(10, 10, 100, 40, 0));
        }
