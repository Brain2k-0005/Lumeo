...
/// <see cref="L.GanttTimeline"/>/<see cref="L.Gantt3"/> actually wire
...
public class GanttV3Phase3T3Tests : IAsyncLifetime
{
    ...
    {
        var cut = _ctx.Render<L.Gantt3>(p => p.Add(c => c.Tasks, HierarchyFixture()));
    ...
    [Fact]
    public void RollupMath_Override_Is_Threaded_Through_Gantt3_Into_The_Rendered_Label()
    {
        var cut = _ctx.Render<L.Gantt3>(p => p
            .Add(c => c.Tasks, HierarchyFixture())
            .Add(c => c.ShowSummaryBars, true)
