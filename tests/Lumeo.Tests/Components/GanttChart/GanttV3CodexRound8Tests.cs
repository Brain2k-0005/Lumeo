...
public class GanttV3CodexRound8Tests : IAsyncLifetime
{
    ...
    // ── GanttChart: recenter when ScrollHostLeadingOffset changes (P2 #1/#2) ────

    [Fact]
    public void GanttChart_Recenters_Using_The_Live_Scroll_Center_Captured_With_The_Old_Leading_Offset_When_ShowTreePane_Changes()
    {
        ...
        // Ground truth: replicate GanttChart's own ComputeInitialRange/Origin math
    ...
    [Fact]
    public void GanttChart_Recenters_On_The_Live_Scroll_Center_Not_Today_When_Direction_Flips_After_A_Manual_Pan()
    {
        ...
        // live reading. Wraps GanttChart in a real DirectionProvider (mirrors
        ...
        var cut = _ctx.Render<L.DirectionProvider>(p => p
            ...
            .AddChildContent<L.GanttChart>(g => g
                .Add(c => c.Tasks, new List<L.GanttTask> { task })
                .Add(c => c.ViewMode, L.GanttViewMode.Day)
                .Add(c => c.ShowTreePane, false)));
        ...
        // The flip itself - LTR -> RTL on the same mounted GanttChart.
        cut.Render(p => p
